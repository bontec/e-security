...
different machines possible?}When you install the system you are asked to setup the root user and one other user account. As Qubes OS is a Fedora based system the user accounts are stored in the \texttt{/etc/passwd} file. This file will have the same format as in Secion 7.2.1 in \cite{GollmannComputerSecurity}. \todo{check usernames in passwd file by cat /etc/passwd}
...
%Opening a console windom in dom0 can be done by:
%\begin{itemize}
...
command line. 
%It is opened automaticly when Qubes OS starts and can
